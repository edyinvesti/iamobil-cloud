...
export async function hydrateAgentFleetFromGateway(params: {
  ...
  agentsResult = {
    ...
    agents: await Promise.all(
      ...
      console.log(`[agent-hydration] Finished seeds mapping. Count: ${agents.length}`);
      return agents.filter((agent) => {
        const isTemp = isTemporarySkillAgentName(agent.name ?? agent.identity?.name);
        if (isTemp) console.log(`[agent-hydration] Filtering out temporary agent: ${agent.name ?? agent.id}`);
        return !isTemp;
      });
    }),
  };
  console.log(`[agent-hydration] Received agentsResult from gateway:`, JSON.stringify(agentsResult.agents.map(a => ({ id: a.id, name: a.name }))));
