    ...
    <div className="fade-up-delay flex flex-col gap-3">
      ...
      <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr]">
        ...
        <label className="flex flex-col gap-1 font-mono text-[10px] font-semibold tracking-[0.06em] text-muted-foreground">
          ...
          <input
            className="ui-input h-10 rounded-md px-4 font-sans text-sm text-foreground outline-none disabled:cursor-not-allowed disabled:bg-muted/20 disabled:text-muted-foreground"
            type="password"
            value={tokenConfigured ? "********" : token}
            onChange={(event) => onTokenChange(event.target.value)}
            placeholder={tokenConfigured ? "Managed by server" : (tokenOptional ? "optional token" : "gateway token")}
            spellCheck={false}
