...
type Action =
  ...
  | { type: "reconcileAgents"; validAgentIds: string[] }
...
const reducer = (state: AgentStoreState, action: Action): AgentStoreState => {
  switch (action.type) {
    ...
    case "reconcileAgents": {
      const validIds = new Set(action.validAgentIds);
      const nextAgents = state.agents.filter((agent) => {
        return validIds.has(agent.agentId);
      });
      ...
      const selectedAgentId =
        state.selectedAgentId &&
        nextAgents.some((agent) => agent.agentId === state.selectedAgentId)
          ? state.selectedAgentId
          : nextAgents[0]?.agentId ?? null;
...
export const AgentStoreProvider = ({ children }: { children: ReactNode }) => {
  ...
  const reconcileAgents = useCallback(
    (validAgentIds: string[]) => {
      dispatch({ type: "reconcileAgents", validAgentIds });
    },
    [dispatch]
  );
  ...
  const value = useMemo(
    () => ({ state, dispatch, hydrateAgents, reconcileAgents, setLoading, setError }),
    [dispatch, hydrateAgents, reconcileAgents, setError, setLoading, state]
  );
