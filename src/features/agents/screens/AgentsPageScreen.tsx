...
import { useAgentsFleetController, type SettingsRouteTab } from "@/features/agents/hooks/useAgentsFleetController";
import { AgentModalsManager } from "@/features/agents/components/AgentModalsManager";
...
const AgentsPageScreen = () => {
  ...
  const { state, dispatch, hydrateAgents, reconcileAgents, setError, setLoading } = useAgentStore();
  const fleetController = useAgentsFleetController({ agents: state.agents });
  const {
    focusFilter,
    setFocusFilter,
    mobilePane,
  ...
  const [settingsSidebarItem, setSettingsSidebarItem] = useState<SettingsRouteTab>("personality");
  ...
  const settingsMutationController = useAgentSettingsMutationController({
    ...
    clearInspectSidebar,
    setInspectSidebarCapabilities: (agentId: string) => {
      setInspectSidebar((current) => {
        if (current?.agentId === agentId) return current;
        return { agentId, tab: "capabilities" };
      });
    },
    dispatchUpdateAgent: (agentId: string, patch: Partial<AgentState>) => {
      dispatch({
        type: "updateAgent",
        agentId,
        patch,
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-background">
              ...
              <aside className="w-[240px] shrink-0 border-r border-border/60">
                ...
                <nav className="py-3">
                  {settingsSidebarEntries.map((entry) => {
                    const active = settingsSidebarItem === entry.id;
                    return (
                      <button
                        key={entry.id}
                        type="button"
      ...
      <AgentModalsManager
        createAgentModalOpen={createAgentModalOpen}
        suggestedCreateAgentName={suggestedCreateAgentName}
        ...
        avatarCreatorAgent={avatarCreatorAgent}
        onCloseAvatarCreator={() => setAvatarCreatorAgentId(null)}
        onAvatarSave={handleAvatarShuffle}
        createAgentBlock={createAgentBlock}
        restartingMutationBlock={restartingMutationBlock}
