    ...
    <section className="flex h-full min-h-0 flex-col">
      <div className="border-b border-cyan-500/10 px-4 py-3">
        ...
        <div className="mt-1 font-mono text-[11px] text-white/40">
          Uso real, gastos e métricas de confiança dos agentes para o QG.
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <div className="font-mono text-[10px] text-white/35">
            {usage.lastRefreshedAt
              ? `Última atualização ${new Date(usage.lastRefreshedAt).toLocaleTimeString()}`
              : "Sem captura analítica ainda"}
          </div>
          <button
            type="button"
        ...
        {usage.budgetAlerts.length > 0 ? (
          ...
          <div className="mt-3 rounded border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 font-mono text-[11px] text-emerald-100">
            Orçamentos estão dentro do limite.
          </div>
        ) : null}
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <StatCard
            ...
            hint="Histórico local da sessão."
          />
        </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                placeholder="Limite flexível"
                inputMode="decimal"
                className="rounded border border-white/10 bg-black/50 px-2 py-2 font-mono text-[11px] text-white/80 outline-none placeholder:text-white/20"
              />
            </label>
            <label className="flex flex-col gap-1">
              <span className="font-mono text-[10px] text-white/35">Limiar de alerta %</span>
              <input
        ...
        <div className="mt-5 rounded border border-white/8 bg-white/[0.03] px-3 py-3">
          ...
          <div className="mt-3 space-y-2">
            ...
            {usage.aggregates.byAgent.length === 0 ? (
              <div className="font-mono text-[11px] text-white/35">Sem dados de gasto por agente ainda.</div>
            ) : null}
          </div>
        </div>
        ...
        <div className="mt-5 rounded border border-white/8 bg-white/[0.03] px-3 py-3">
          ...
          <div className="mt-3 space-y-2">
            ...
            {usage.aggregates.byModel.length === 0 ? (
              <div className="font-mono text-[11px] text-white/35">Sem dados de uso por modelo ainda.</div>
            ) : null}
          </div>
        </div>
        ...
        <div className="mt-5 rounded border border-white/8 bg-white/[0.03] px-3 py-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <StatCard
              ...
              hint="Solicitações na sessão atual."
            />
            <StatCard
              label="Taxa de Intervenção"
          ...
          <div className="mt-4 space-y-2">
            ...
            {performance.rows.length === 0 ? (
              <div className="font-mono text-[11px] text-white/35">
                Sem dados de desempenho disponíveis ainda.
              </div>
            ) : null}
          </div>
        </div>
