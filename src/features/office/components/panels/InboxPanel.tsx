...
const formatRelativeTime = (timestampMs: number | null) => {
  ...
  if (deltaMs < 3_600_000) return `${Math.max(1, Math.floor(deltaMs / 60_000))}min atrás`;
    ...
    <section className="flex h-full min-h-0 flex-col">
      <div className="border-b border-cyan-500/10 px-4 py-3">
        ...
        <div className="mt-1 font-mono text-[11px] text-white/40">
          Últimas respostas dos assistentes em cada mesa.
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        {sortedAgents.length === 0 ? (
          ...
          sortedAgents.map((agent) => {
            const preview = agent.latestPreview?.trim() || "Nenhuma resposta concluída ainda.";
