...
import { useOfficeOrchestrator } from "../hooks/useOfficeOrchestrator";
import { OfficeOverlays } from "../components/OfficeOverlays";
import { useRouter, useSearchParams } from "next/navigation";
import { MessageSquare, ChevronDown, Mic, Box, Monitor } from "lucide-react";
import { RetroOffice3D } from "@/features/retro-office/RetroOffice3D";
import { OfficePhaserCanvas } from "../components/OfficePhaserCanvas";
...
import { createEmptyOfficeMap, normalizeOfficeMap } from "@/lib/office/schema";
...
const resolveOfficeMutationGuardMessage = (guardReason?: string) => {
  if (guardReason === "not-connected") {
    return "Conecte-se ao gateway antes de alterar a frota do escritório.";
  }
  if (guardReason === "create-block-active") {
    return "Finalize a criação do agente ativo antes de iniciar outra alteração.";
  }
  if (guardReason === "rename-block-active") {
    return "Finalize a renomeação ativa antes de alterar a frota.";
  }
  if (guardReason === "delete-block-active") {
    return "Finalize a exclusão ativa antes de alterar a frota.";
  }
  return "O escritório está ocupado no momento.";
};
...
const inferRunningFromAgentSessions = async (params: {
  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error("Timeout (agent inference)")), timeoutMs),
  );
  ...
  try {
    return (await Promise.race([runOperation(), timeoutPromise])) as any;
  } catch (err) {
    return {
      inferredRunning: false,
      sessionKey: "",
      ...
      messageCount: 0,
      latestSessionUpdatedAtMs: 0,
      inspectedSessions: [],
      inferenceSource: "fallback-timeout",
    };
  }
};
...
export function OfficeScreen({
  ...
  const {
    ...
    reconcileTriggers,
  } = useOfficeOrchestrator({
    agents: state.agents,
    remoteAgents: remoteOfficeAgents,
    marketplaceGymHoldByAgentId,
    danceUntilByAgentId,
    movementTargetByAgentId: skillTriggers.movementTargetByAgentId,
  ...
  const loadAgents = useCallback(async (options?: {
    ...
    const task = (async () => {
      ...
      try {
        ...
        if (debugEnabled) {
          console.info("[office-debug] Starting bootstrap load operation...");
        }
        ...
        if (debugEnabled) {
          console.info("[office-debug] Bootstrap load commands received.", commands);
        }
        ...
        console.info("[office-debug] Setting agentsLoaded to true.");
    ...
    const unsubscribeEvent = client.onEvent((event) => {
      ...
      if (
        shouldSuppressPhoneBoothAssistantReply({
          event,
          agents: stateRef.current.agents,
          phoneCallByAgentId: officeAnimationState.phoneCallByAgentId,
        })
      ) {
        return;
      }
  ...
  useEffect(() => {
    reconcileTriggers(state.agents);
  }, [state.agents, reconcileTriggers]);
  ...
  return (
    <main className="relative h-full w-full overflow-hidden bg-black">
      <section className="relative h-full min-h-0 min-w-0 overflow-hidden">
        <div className="absolute left-4 top-4 z-50 flex gap-2">
          <button
            ...
            className={`flex h-8 w-8 items-center justify-center rounded-md border bg-background/80 transition-all hover:bg-accent ${
              viewMode === "3d" ? "border-primary text-primary" : "border-transparent text-muted-foreground"
            }`}
            title="Vista 3D"
          >
            <Box size={16} />
          ...
          <button
            ...
            className={`flex h-8 w-8 items-center justify-center rounded-md border bg-background/80 transition-all hover:bg-accent ${
              viewMode === "2d" ? "border-primary text-primary" : "border-transparent text-muted-foreground"
            }`}
            title="Vista Pixel (2D)"
          >
            <Monitor size={16} />
          </button>
        </div>
        ...
        {viewMode === "2d" ? (
          <div className="h-full w-full bg-[#1a1a1a] p-4 flex items-center justify-center">
             <OfficePhaserCanvas
                mode="viewer"
                map={normalizeOfficeMap(null, createEmptyOfficeMap({ workspaceId: "default", officeVersionId: "v1", width: 1200, height: 800 }))}
                presence={allVisibleAgents.map(a => ({
                   agentId: a.id,
                   name: a.name,
                   state: a.status === 'working' ? 'working' : a.status === 'error' ? 'error' : 'idle',
                }))}
                debug={{ showZones: false, showAnchors: false, showEmitterBounds: false, showLightBounds: false, showMetrics: false }}
                runtime={{ enableAmbience: true, enableThoughtBubbles: true, enableLighting: true }}
             />
          </div>
        ) : (
          <RetroOffice3D
      ...
      <OfficeOverlays
        showGatewayLoadingOverlay={showGatewayLoadingOverlay}
        showGatewayConnectOverlay={showGatewayConnectOverlay}
        status={status}
        gatewayUrl={gatewayUrl}
        token={token}
        ...
        didAttemptGatewayConnect={didAttemptGatewayConnect}
        onGatewayUrlChange={setGatewayUrl}
        ...
        emptyFleetMessage={emptyFleetMessage}
        deleteAgentStatusLine={deleteAgentStatusLine}
        ...
        onCloseMarketplace={() => setMarketplaceOpen(false)}
        onSelectAgent={(id) => {
          handleOpenAgentChat(id);
        ...
        companyCreatedSignal={companyCreatedSignal}
        createdCompanyName={createdCompanyName}
        onCompleteOnboarding={handleCompleteOnboarding}
        onOpenOnboarding={handleOpenOnboarding}
        showOpenClawConsole={showOpenClawConsole}
        openClawConsoleCollapsed={openClawConsoleCollapsed}
        openClawConsoleSearch={openClawConsoleSearch}
        openClawConsoleCopyStatus={openClawConsoleCopyStatus}
        filteredOpenClawLogEntries={filteredOpenClawLogEntries as any}
        openClawLogEntries={openClawLogEntries as any}
        openClawLiveStateMatchesSearch={openClawLiveStateMatchesSearch}
        openClawLiveStateText={openClawLiveStateText}
        onSetOpenClawConsoleCollapsed={setOpenClawConsoleCollapsed}
        onSetOpenClawConsoleSearch={setOpenClawConsoleSearch}
        onCopyOpenClawConsoleJson={() => void handleCopyOpenClawConsoleJson()}
        onDownloadOpenClawConsoleJson={handleDownloadOpenClawConsoleJson}
        onClearOpenClawConsole={handleClearOpenClawConsole}
        renderOpenClawHighlightedText={renderOpenClawHighlightedText}
        chatOpen={chatOpen}
        onToggleChat={() => setChatOpen((prev) => !prev)}
        chatRosterEntries={chatRosterEntries}
        selectedChatAgentId={selectedChatAgentId}
        focusedChatAgent={focusedChatAgent}
        focusedRemoteChatTarget={focusedRemoteChatTarget}
        focusedRemoteChatState={focusedRemoteChatState}
        chatController={chatController}
        gatewayModels={gatewayModels}
        remoteMessagingAvailable={remoteMessagingAvailable}
        remoteMessagingDisabledReason={remoteMessagingDisabledReason}
        onOpenAgentChat={handleOpenAgentChat}
        onChatSend={handleChatSend}
        onVoiceSend={handleVoiceSend}
        updateRemoteChatSession={updateRemoteChatSession}
        dispatch={dispatch}
        mainVoiceState={mainVoiceState}
        mainVoiceError={mainVoiceError}
        mainVoiceSupported={mainVoiceSupported}
        debugEnabled={debugEnabled}
        debugRows={debugRows}
        agentEditorAgent={agentEditorAgent}
        agentEditorInitialSection={agentEditorInitialSection}
        onCloseAgentEditor={() => setAgentEditorAgentId(null)}
        onAvatarSave={handleAvatarProfileSave}
        onRenameAgent={async (agentId, name) => {
        ...
        onNavigateAgent={openAgentEditor}
        createAgentWizardNonce={createAgentWizardNonce}
        createAgentWizardOpen={createAgentWizardOpen}
        createAgentBusy={createAgentBusy}
        createAgentModalError={createAgentModalError}
        createAgentStatusLine={createAgentStatusLine}
        onCloseCreateAgentWizard={handleCloseCreateAgentWizard}
        onCreateAgentFromIdentity={handleCreateAgentFromIdentity}
        onFinishCreateAgentAvatar={handleFinishCreateAgentAvatar}
        companyBuilderNonce={companyBuilderNonce}
        companyBuilderOpen={companyBuilderOpen}
        companyBuilderBusy={companyBuilderBusy}
        companyBuilderError={companyBuilderError}
        companyBuilderStatusLine={companyBuilderStatusLine}
        companyBuilderInput={companyBuilderInput}
        lastCompanyPlan={lastCompanyPlan}
        plannerAgent={plannerAgent}
        onCloseCompanyBuilder={handleCloseCompanyBuilder}
        onClearCompanyBuilder={handleClearCompanyBuilder}
        onImproveCompanyBrief={handleImproveCompanyBrief}
        onGenerateCompanyPlan={handleGenerateCompanyPlan}
        onCreateCompanyFromPlan={handleCreateCompanyFromPlan}
        kanbanInstallPromptOpen={kanbanInstallPromptOpen}
        kanbanInstallProgress={kanbanInstallProgress}
        onCloseKanbanInstall={() => {
          if (kanbanInstallProgress.active) return;
          setKanbanInstallPromptOpen(false);
        }}
        onInstallKanban={async () => {
          const targetAgentId =
