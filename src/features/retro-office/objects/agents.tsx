...
export const AgentModel = memo(function AgentModel({
  ...
  const MAX_3D_SPEECH_LENGTH = 140;
  const isTruncated = activeSpeechBubble && resolvedSpeechText.length > MAX_3D_SPEECH_LENGTH;
  const normalizedSpeechBubbleText = activeSpeechBubble
    ? resolvedSpeechText.replace(/\s+/g, " ").trim()
    : resolvedSpeechText;
  const speechBubbleDisplayText = isTruncated
    ? normalizedSpeechBubbleText.substring(0, MAX_3D_SPEECH_LENGTH) + "..."
    : normalizedSpeechBubbleText;
  const speechBubbleTextLength = speechBubbleDisplayText.length;
  const speechBubbleWidth = activeSpeechBubble
    ? Math.min(3.2, Math.max(1.6, 1.2 + speechBubbleTextLength * 0.016))
    : 0.36;
  ...
  const speechBubbleHeight = activeSpeechBubble
    ? Math.min(1.8, Math.max(0.64, estimatedSpeechLines * 0.22 + speechBubblePaddingY))
    : 0.2;
