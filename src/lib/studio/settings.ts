...
const DEFAULT_OFFICE_TITLE = "iAmobil";
...
const normalizeRemoteOfficeSourceKind = (
  ...
  if (kind === "presence_endpoint" || kind === "hermes_gateway" || kind === "openclaw_gateway") {
    return kind;
  }
  return fallback;
};

const normalizeViewModePreference = (
  value: unknown,
  fallback: StudioOfficePreference["viewMode"] = "3d",
): StudioOfficePreference["viewMode"] => {
  const mode = coerceString(value);
  if (mode === "3d" || mode === "2d") {
    return mode;
  }
...
const normalizeGatewayProfile = (value: unknown): StudioGatewayProfile | null => {
  ...
  if (!url) return null;
  const token = coerceString(value.token);
  return { url, token, tokenConfigured: !!token };
};
...
const normalizeGatewayProfiles = (
  ...
  for (const adapterType of ["openclaw", "demo", "custom"] as const) {
...
const normalizeGatewayConnectionState = (
  ...
  const token = coerceString(value.token);
  const adapterType = normalizeGatewayAdapterType(value.adapterType);
  return { url, token, tokenConfigured: !!token, adapterType };
};
...
const mergeGatewayProfiles = (
  ...
  for (const adapterType of ["openclaw", "demo", "custom"] as const) {
    ...
    next[adapterType] = { url: nextUrl, token: nextToken, tokenConfigured: !!nextToken };
  }
...
export const normalizeGatewayAdapterType = (
  value: unknown,
...
const normalizeOfficePreference = (
  ...
  return {
    ...
    viewMode: normalizeViewModePreference(value.viewMode, fallback.viewMode),
  };
};
