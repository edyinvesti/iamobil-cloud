...
): { url: string; token: string; tokenConfigured: boolean } => {
  switch (adapterType) {
    case "custom":
    ...
    default:
      return {
        ...
        tokenConfigured: Boolean(localDefaults?.token) || false,
      };
  }
};
...
const normalizeLocalGatewayDefaults = (value: unknown): StudioGatewaySettings | null => {
  ...
  return { url, token, adapterType, tokenConfigured: !!raw.tokenConfigured, ...(profiles ? { profiles } : {}) };
};

const normalizeGatewayProfilePublic = (
  value: unknown
): { url: string; token: string; tokenConfigured: boolean } | null => {
  if (!value || typeof value !== "object") return null;
  const raw = value as { url?: unknown; tokenConfigured?: unknown };
  const url = typeof raw.url === "string" ? raw.url.trim() : "";
  if (!url) return null;
  return { url, token: "", tokenConfigured: !!raw.tokenConfigured };
};

const normalizeGatewayProfilesPublic = (
  value: unknown
): Partial<Record<StudioGatewayAdapterType, { url: string; token: string; tokenConfigured: boolean }>> | undefined => {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Partial<Record<StudioGatewayAdapterType, StudioGatewayProfilePublic>>;
  const profiles: Partial<Record<StudioGatewayAdapterType, { url: string; token: string; tokenConfigured: boolean }>> = {};
...
export const useGatewayConnection = (
  ...
  const setSelectedAdapterType = useCallback(
    (value: StudioGatewayAdapterType) => {
      ...
      setTokenConfigured(!!profile.tokenConfigured);
      setError(null);
    ...
    const loadSettings = async () => {
      try {
        ...
        const profiles: Partial<
          Record<StudioGatewayAdapterType, { url: string; token: string; tokenConfigured?: boolean }>
        > = {};
        if (gateway?.profiles) {
          for (const [type, p] of Object.entries(gateway.profiles)) {
            profiles[type as StudioGatewayAdapterType] = {
              url: p.url,
              token: "",
            ...
            ? {
                url:
                  typeof gateway.lastKnownGood.url === "string"
                    ...
                    : "",
                token: "",
                tokenConfigured: Boolean(gateway.lastKnownGood.tokenConfigured),
                adapterType:
        ...
        loadedGatewaySettings.current = {
          ...
          token: nextToken,
          tokenConfigured: Boolean(selectedProfile.tokenConfigured),
        ...
        setToken(nextToken);
        setTokenConfigured(Boolean(selectedProfile.tokenConfigured));
  ...
  useEffect(() => {
    ...
    setAdapterProfiles((current) => {
      ...
      if (
        ...
        existing.tokenConfigured === nextProfile.tokenConfigured
      ) {
        return current;
      }
      return {
  ...
  }, [gatewayUrl, selectedAdapterType, settingsLoaded, token, tokenConfigured]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      tokenConfigured === baseline.tokenConfigured &&
  ...
  }, [adapterProfiles, gatewayUrl, selectedAdapterType, settingsCoordinator, settingsLoaded, token, tokenConfigured]);
