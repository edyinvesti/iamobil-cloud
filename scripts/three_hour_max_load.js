...
// CONFIGURAÇÃO DO TESTE DE ESTRESSE MÁXIMO (3 HORAS)
const TOTAL_DURATION_MS = 3 * 60 * 60 * 1000; // 3 HORAS
const INTERVAL_MS = 2000; // A cada 2 segundos injeta um evento (EXTREMO)
const START_TIME = Date.now();

console.log(`\n🔥 [3-HOUR-MAX-LOAD-STRESS] Iniciando Motor de Stress Extremo...`);
console.log(`⏳ Duração: 3 Horas`);
...
const CHAOS_EVENTS = [
    ...
    },
    {
        ...
        weight: 0.15,
        action: async () => {
             // Avoid dependency issues if multiposter_engine is not exported correctly
             try {
                ...
                console.error("Multiposter Fail:", e.message);
             }
        }
    },
    {
...
async function run() {
    while (Date.now() - START_TIME < TOTAL_DURATION_MS) {
        ...
        if (successCount % 10 === 0) {
            ...
            process.stdout.write(` [RESTANTE: ${remaining} min | SUCESSO: ${successCount} | ERRO: ${errorCount}]\n`);
        }
    ...
    console.log(`\n🏆 [3-HOUR-MAX-LOAD-STRESS] TESTE CONCLUÍDO COM SUCESSO!`);
