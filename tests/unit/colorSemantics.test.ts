...
describe("colorSemantics", () => {
  it("maps agent statuses to semantic badge classes and labels", () => {
    expect(AGENT_STATUS_LABEL.idle).toBe("Ocioso");
    expect(AGENT_STATUS_LABEL.running).toBe("Executando");
    expect(AGENT_STATUS_LABEL.error).toBe("Erro");
    ...
    expect(resolveAgentStatusLabel("idle")).toBe("Ocioso");
  ...
  it("maps gateway statuses to semantic badge classes and labels", () => {
    expect(GATEWAY_STATUS_LABEL.disconnected).toBe("Desconectado");
    expect(GATEWAY_STATUS_LABEL.connecting).toBe("Conectando");
    expect(GATEWAY_STATUS_LABEL.connected).toBe("Conectado");
    ...
    expect(resolveGatewayStatusLabel("connected")).toBe("Conectado");
