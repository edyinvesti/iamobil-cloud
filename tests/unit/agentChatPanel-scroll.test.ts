...
describe("AgentChatPanel scrolling", () => {
  ...
  it("shows history truncation banner only when scrolled to top", () => {
    ...
    fireEvent.scroll(scrollEl);
    expect(screen.queryByText(/Mostrando as .* mensagens mais recentes/i)).not.toBeInTheDocument();

    scrollEl.scrollTop = 0;
    fireEvent.scroll(scrollEl);
    expect(screen.getByText(/Mostrando as .* mensagens mais recentes/i)).toBeInTheDocument();
  });
});
